                        ...
                        {education.map((item, index) => (
                            <div key={index} className="py-8 flex flex-wrap md:flex-nowrap">
                                <div className="md:w-64 md:mb-0 mb-6 flex-shrink-0 flex flex-col">
                                    <span className="font-semibold title-font text-gray-200 text-2xl">{item.category || ''}</span>
                                    <span className="mt-1 text-gray-500 text-md">{item.date_from || ''}-{item.date_to || ''}</span>
                                </div>
                                <div className="md:flex-grow">
                                    <h2 className="font-medium text-gray-100 text-2xl title-font mb-2">{item.title || ''}</h2>
                                    <p className="leading-relaxed text-xl text-gray-300">{item.description || ''}</p>
                                </div>
                            </div>
                        ))}
                    </div>
